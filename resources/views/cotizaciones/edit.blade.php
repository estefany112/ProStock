...
<div class="min-h-screen bg-mesh py-12 px-6 text-slate-100 font-sans">
    <div class="max-w-6xl mx-auto">
        ...
        <header class="flex justify-between items-center mb-8">
            <div>
                <span class="px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-black uppercase tracking-[0.2em] mb-2 inline-block">
                    Modificación de Registro — PROSTOCK
                </span>
        ...
        <form action="{{ route('cotizaciones.update', $cotizacion->id) }}" method="POST" id="cotizacionForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="fecha_emision" value="{{ old('fecha_emision', $cotizacion->fecha_emision) }}" class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white font-mono outline-none shadow-inner" required>
                    </div>
                </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl">
                ...
                <div class="border-t border-white/5 pt-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                    <div class="md:col-span-7">
                        ...
                        <input type="text" name="total_letras" id="input_total_letras" value="{{ old('total_letras', $cotizacion->total_letras) }}" readonly
                    ...
                    <div class="md:col-span-5 bg-slate-950/80 border border-white/10 p-5 rounded-2xl space-y-3 shadow-2xl">
                        <input type="hidden" name="subtotal" id="input_subtotal" value="{{ old('subtotal', $cotizacion->subtotal) }}">
                        <input type="hidden" name="total" id="input_total" value="{{ old('total', $cotizacion->total) }}">
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl">
                ...
                <input type="text" name="lugar_entrega" value="{{ old('lugar_entrega', $cotizacion->lugar_entrega) }}" placeholder="Ej. Instalaciones de la Planta Hidroeléctrica Jurún Marinalá..." class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white outline-none text-sm focus:border-fuchsia-500 transition-all">
            </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl">
                ...
                <input type="text" name="tiempo_entrega" value="{{ old('tiempo_entrega', $cotizacion->tiempo_entrega) }}" placeholder="Ej. 10 días hábiles posteriores a recibir Orden de Compra..." class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white outline-none text-sm focus:border-fuchsia-500 transition-all">
            </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl">
                ...
                <input type="text" name="garantia" value="{{ old('garantia', $cotizacion->garantia) }}" placeholder="Ej. 12 meses contra defectos de fabricación o fallas de montaje..." class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white outline-none text-sm focus:border-fuchsia-500 transition-all">
            </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl">
                ...
                <input type="text" name="forma_pago" value="{{ old('forma_pago', $cotizacion->forma_pago) }}" placeholder="Ej. Crédito a 30 días contra entrega de factura comercial..." class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white outline-none text-sm font-bold text-fuchsia-400 focus:border-fuchsia-500 transition-all">
            </div>
            ...
            <div class="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-xl shadow-xl">
                ...
                <input type="text" name="validez_oferta" value="{{ old('validez_oferta', $cotizacion->validez_oferta) }}" placeholder="Ej. 30 días calendario a partir de a fecha del presente documento..." class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white outline-none text-sm focus:border-fuchsia-500 transition-all">
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="clausula_despedida" rows="2" class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white text-sm resize-none outline-none focus:border-fuchsia-500 transition-all">{{ old('clausula_despedida', $cotizacion->clausula_despedida) }}</textarea>
                        </div>
                        <div>
                            ...
                            <input type="text" name="nombre_firmante" value="{{ old('nombre_firmante', $cotizacion->nombre_firmante) }}" class="w-full bg-slate-950/60 border border-white/10 p-3.5 rounded-xl text-white text-sm font-bold outline-none focus:border-fuchsia-500 transition-all">
                        </div>
                    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Cargar ítems comerciales guardados
        ...
        // Cargar detalles técnicos guardados con Trix
    ...
    function inyectarItem(cant, um, desc, precio) {
        const html = `
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 bg-white/[0.01] md:bg-transparent p-4 md:p-0 rounded-2xl border border-white/5 md:border-none item-row items-center" id="row_${itemIndex}">
                ...
                <div class="md:col-span-2">
                    <input type="text" name="items[${itemIndex}][unidad_medida]" value="${um}" placeholder="Ej. Servicio" class="w-full bg-slate-950/40 border border-white/10 p-2.5 rounded-xl text-white text-xs" required>
                </div>
                <div class="md:col-span-5">
                    <input type="text" name="items[${itemIndex}][descripcion]" value="${desc}" placeholder="Escribe el concepto técnico..." class="w-full bg-slate-950/40 border border-white/10 p-2.5 rounded-xl text-white text-xs" required>
                </div>
                <div class="md:col-span-2">
        ...
        if (tipo === 'servicio') {
            ...
            const html = `
                <div class="detalle-item bg-white/[0.03] border border-white/10 p-4 rounded-xl">
                    ...
                    <trix-editor
                        ...
                        class="bg-slate-950/40 border border-white/10 rounded-xl text-white trix-content"
                    ></trix-editor>
                </div>`;
            ...
            const html = `
                <div class="detalle-item bg-white/[0.03] border border-white/10 p-4 rounded-xl">
                    ...
                    <textarea name="detalles[${detalleIndex}][descripcion]" class="w-full bg-slate-950/40 border border-white/10 p-3 rounded-xl text-white text-xs outline-none focus:border-emerald-500" rows="2" placeholder="Especificar marcas, repuestos y materiales a usar..." required>${desc}</textarea>
                </div>`;
            container.insertAdjacentHTML('beforeend', html);
            detalleIndex++;
        }
